#include "../inc/imgProcess.h"

using namespace std;
using namespace cv;

Mat getImgArray(const string& address,lv_img_header_t &Output,uint8_t * &data,ENCODE_MODE encode_mode)
{
    Mat img_read = imread(address,IMREAD_COLOR);
    Mat img;
    vector<Mat> img_chanels;
    split(img_read,img_chanels);
    img= 0.299*img_chanels[0]+0.587*img_chanels[1]+0.114*img_chanels[2];
    if(encode_mode==ENCODE_DITHER_INDEX){
    ...
    else if(encode_mode==ENCODE_NO_DITHER_INDEX){
        for(int32_t r=0;r<img.rows;r++)
        {
            for(int32_t c=0;c<img.cols;c++)
            {
                if(img.at<uint8_t>(r,c)>128)
                {
    ...
    if(encode_mode==ENCODE_DITHER)
    {
        ...
        memcpy_s(data,img.total(),img.data,img.total());
        Output.h = img.rows;
        Output.w = img.cols;
    ...
    else if(encode_mode==ENCODE_DITHER_INDEX||encode_mode==ENCODE_NO_DITHER_INDEX)
    {
        long step = ceil(img.cols/8.0);
        data = (uchar*)calloc((img.rows*step+8),sizeof(uchar));
        uchar index_2[]={0xff, 0xff, 0xff,0xff};
        uchar index_1[]={0x00, 0x00, 0x00,0xff};//ºÚ
        memcpy_s(data,4*sizeof(uchar),index_1,4*sizeof(uchar));
        memcpy_s(data+4,4*sizeof(uchar),index_2,4*sizeof(uchar));
        uchar *pt;
...
//            waitKey();
        ...
        else if(output_header.cf==4)//LV_IMG_CF_TRUE_COLOR
        {
            if(decode_mode==DECODE_DITHER)
            {
                ...
                memcpy_s(temp_mat.data,temp_mat.total(),data,output_header.w*output_header.h*sizeof(uint8_t));
