...
static Heap_Typedef *heap_init(uint32_t size) {
    ...
    if (HeapManager.is_init == 0) {
        ...
        HeapManager.data_heap = lv_mem_alloc(sizeof(uint8_t **));
...
static uint8_t *heap_push(void *data, uint32_t size) {
    ...
    if (HeapManager.total - HeapManager.index - 1 < size) {
        ...
        HeapManager.data_heap = lv_mem_realloc(HeapManager.data_heap,HeapManager.sec_total*sizeof(uint8_t **));
        if(HeapManager.data_heap){
...
void GUIInit(void){
    ...
    lv_style_set_radius(&style_screen_list_extra_btns_main_default, 2);
    ...
    lv_style_set_radius(&style_screen_list_extra_btns_main_pressed, 2);
    ...
    lv_style_set_radius(&style_screen_list_extra_btns_main_focused, 2);
    ...
    lv_style_set_outline_pad(&style_screen_list_extra_btns_main_focused, 1);
}