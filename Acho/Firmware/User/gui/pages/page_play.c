...
static void click_event(lv_event_t *e) {
    ...
    lv_obj_t *label_size = lv_obj_get_child(target,1);
    if (e->code == LV_EVENT_LONG_PRESSED) {
...
//                    snprintf(path,50,"P:Audios\\%s",name);
            ...
            lv_label_set_long_mode(label_size, LV_LABEL_LONG_SCROLL_CIRCULAR); // only 'label' obj can use this function!
...
static void setup_screen(void *user_data) {
    ...
    lv_obj_set_style_pad_row(div, 2, LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_bg_color(div, lv_color_make(0x0, 0x0, 0xff), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_border_width(div, 1, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_pad_row(screen_list, 2, LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(screen_list, 1, LV_PART_MAIN);
    lv_obj_set_style_pad_left(screen_list, 2, LV_PART_MAIN);
    lv_obj_set_style_pad_right(screen_list, 4, LV_PART_MAIN);
    lv_obj_set_style_pad_top(screen_list, 2, LV_PART_MAIN);
    lv_obj_set_size(screen_list, LV_PCT(100), LV_PCT(80));
    ...
    lv_obj_set_scrollbar_mode(screen_list, LV_SCROLLBAR_MODE_ACTIVE);
    ...
    if (state == FR_OK) {
        while (true) {
            ...
            if (state == FR_OK) {
                ...
                lv_obj_set_size(file_div, LV_PCT(100), 18);
...
//                lv_label_set_long_mode(title, LV_LABEL_LONG_SCROLL);
                lv_obj_set_style_pad_all(label_name,0,LV_PART_MAIN);
                lv_obj_set_style_pad_all(label_size,0,LV_PART_MAIN);
                ...
                lv_label_set_long_mode(label_size,LV_LABEL_LONG_DOT);
                if(is_audio)
                    lv_obj_set_user_data(file_div, HeapManager.push(&file_count,2)); // set index by count
